...
const CreateCapa = ({ createCapaHandle }) => {
  ...
  function crearAux() {
      let nombre = intl.formatMessage({ id: 'createcapas.layer', defaultMessage: 'New Layer' })
  ...
  return (
    <div className="d-flex justify-content-between align-items-center">
      <span className="w-100">
        ...
        <input className="form-control-sm w-100" 
        value={capaName} 
        placeholder={ intl.formatMessage({ id: 'createcapas.layer', defaultMessage: 'New Layer' }) }
        onChange={e => setCapaName(e.target.value)}  
        onKeyDown={handleAdd}/>
   
      </span>
      <span className="flex-shrink text-end ps-2">
        <Agregar onClick={handleAdd}  style={{ width:'22px',cursor:'pointer' }}/>
      </span>
    </div>
  );
};
