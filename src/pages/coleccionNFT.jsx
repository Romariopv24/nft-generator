...
import { myHeaders, URL } from '../constantes'
...
const ColeccionNFT = () => {
  ...
  const codeMessage = [
    ...
    intl.formatMessage({id: "colleccionNFT.modal-export-msg-success", defaultMessage:"In a few minutes you will see your collection on artis.market"}),
    intl.formatMessage({id: "colleccionNFT.modal-export-msg-error", defaultMessage:"You have run an unexpected error"}),
    intl.formatMessage({id: "colleccionNFT.modal-export-msg-wait", defaultMessage: "Processing please wait a moment"}),
  ]
  ...
  function date(date) {
    if (isWalletPremiun === true){
      return intl.formatMessage({ id: 'colleccionNFT.table-column51', defaultMessage: 'No Deadline' })
    }
    return dayjs(date).add(15, 'day').fromNow().replace('in ', intl.formatMessage({ id: 'colleccionNFT.in', defaultMessage: 'in ' })).replace('days', intl.formatMessage({ id: 'colleccionNFT.days', defaultMessage: 'days' }))  
  }
  ...
  const handleDelete = async () => {
    if (selectedItem) {
      const res = await fetch(`${URL}/delete/${selectedItem}`, { method: "GET", headers: myHeaders })
      const jsonres = await res.json()
      if (jsonres) {
        const tempArray = JSON.parse(JSON.stringify(collectall))
        const eliminatedItem = tempArray.findIndex(item => item._id["$oid"] === selectedItem)
    ...
    {/* tabla pc */}
      <div className="container px-0 text-center my-5 d-none d-md-block border border-white">
        <table className="container my-2 py-1 mx-auto table-pc"  >
          ...
          {!isPromiseReady ?
            <thead><tr><th colSpan="5"><h1> <FormattedMessage id="colleccionNFT.loading" defaultMessage="loading"/> </h1></th></tr></thead>
            :
            <tbody className='my-0'>
              {collectall.length ? collectall.map((colletion, key) => (
                <tr key={key} className='align-middle' >
                  ...
                  <td className='py-2'>
                    {colletion.url === "En Proceso..." ?
                      ...
                      :
                      <Export style={{ width: '32px', cursor: "pointer" }} fill={'#fff'} onClick={() => handleOpenModalCodigo(colletion.url)} />
                    }
                  </td>
                  ...
                  <td className='py-2'>
                    {colletion.url !== "En Proceso..."
                      &&
                      <Delete width={30} height={30} fill="#ec2d01" style={{ cursor: "pointer", }} onClick={() => handleOpenModalEliminar(colletion._id["$oid"])} />
                    }
                  </td>
                </tr>
              )) : <tr>
                    ...
                    <div className='p-5'>
                      <h1 className='mb-3'> 
                      <FormattedMessage id="colleccionNFT.nocollections" defaultMessage="You don't have collections"/>
                      </h1>
                      <Link to={`/`}>
                      <button className="__boton-mediano enphasis-button">
                        <FormattedMessage id="colleccionNFT.go-to-generator" defaultMessage="Create your first collection"/>
                      </button>
                      </Link>
                    </div>
                </th>
  ...
  {/* tablas celulares/tablets */}
      <div className='container d-block d-md-none my-5' style={{backgroundColor: 'transparent'}}>
        {collectall.length < 1 ? <h1 className='my-0 text-center border border-white p-2 rounded-4'> 
        <FormattedMessage id="colleccionNFT.loading" defaultMessage="loading"/>
        </h1> :
          collectall.err !== "sin colecciones" ? collectall?.map((colletion, key) => (
            <div className="container my-2 py-1 mx-auto table-mobile  border border-white" key={key}>
              <div className="row py-1 align-items-center justify-content-center">
                <div className="col text-center">
                  <FormattedMessage id='colleccionNFT.table-column1' defaultMessage="name"/>
                </div>
              ...
              <div className="row py-1 align-items-center">
                <div className="col text-center py-1">
                  <FormattedMessage id="colleccionNFT.table-column2" defaultMessage="amount"/>
                </div>
                <div className="col text-center">
                  {colletion.cantidad}
                </div>
              ...
              <div className="row py-1 align-items-center">
                <div className="col text-center py-1">
                <FormattedMessage id="colleccionNFT.table-column3" defaultMessage="download"/>
                </div>
                <div className="col text-center py-1">
                  {colletion.url === "En Proceso..." ?
                    'In process...' :
                    <a href={colletion.url}><Descargar style={{ width: '32px' }} fill={'#fff'} /></a>
                  }
                </div>
              </div>
              <div className="row py-1 align-items-center">
                <div className="col text-center py-1">
                <FormattedMessage id="colleccionNFT.table-column4" defaultMessage="Export to artis"/>
                </div>
                <div className="col text-center py-1">
                  {colletion.url === "En Proceso..." ?
                    <Export style={{ width: '32px' }} fill={'#494b50'} />
              ...
              <div className="row py-1 align-items-center">
                <div className="col text-center py-1">
                <FormattedMessage id="colleccionNFT.table-column5" defaultMessage="deadline"/>
                </div>
                <div className="col text-center py-1">
              ...
              <div className="row py-1 align-items-center">
                <div className="col text-center py-1">
                <FormattedMessage id="colleccionNFT.table-column6" defaultMessage="Weight"/>
                </div>
                <div className="col text-center py-1">
                  {colletion.peso}
                </div>
      ...
      <GenericModal show={showModalEliminar}>
        ...
        <button className='__boton-mediano' onClick={() => setShowModalEliminar(false)}>
          <FormattedMessage 
      ...
      <GenericModal show={showModalCodigo}>
        <div className='p-4'>
          <div className="mb-3">
            ...
            <input type="text" className="form-control --modificadorForm"
              name='categoria'
        ...
        <button className='__boton-mediano-borrar --envair mb-3' onClick={handleEnviarCode}>
          <FormattedMessage 
            id='colleccionNFT.modal-export-btn-send' 
            defaultMessage="send"
          />
        </button>
        <button className='__boton-mediano mb-3' onClick={() => setShowModalCodigo(false)}>
        <FormattedMessage 
