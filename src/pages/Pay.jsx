...
const Pay = () => {
  ...
  const { idHash = 'no', name = 'no', qtty = 'no' } = JSON.parse(localStorage.getItem('pay'))
      ...
      {isPay === true ?
        (
          <div className="d-flex flex-column justify-content-center align-items-center mt-5" >
            <h1><FormattedMessage id="pay.made" defaultMessage="Congratulations, your payment has been successfully completed."/></h1>
            <Check fill="#20c997" style={{ width: '25%'}} />
            <button 
              className="m-2 p-2 btn btn-primary"
              onClick={rediredAtHome}>
                <FormattedMessage id="pay.btn-collections" defaultMessage="See my collections"/>
            </button>
          </div>
        )
        :
        (
          <div className="d-flex flex-column justify-content-center align-items-center mt-5 ">
            <h1> <FormattedMessage id="pay.processing" defaultMessage="Processing your payment"/> </h1>
            <div className="spinner-grow text-secondary" style={{ width: '15rem', height: '15rem' }} role="status">
              <span className="visually-hidden"> <FormattedMessage id="pay.loading" description="Loading..."/></span>
            </div>
          </div>
        )
      }
